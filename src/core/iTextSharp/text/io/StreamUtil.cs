...
namespace iTextSharp.text.io {
    ...
    public static class StreamUtil {
        ...
        public static Stream GetResourceStream(Assembly assm, string key)
        {
            Stream istr = null;
            // Try to use resource loader to load the properties file.
            try
            {
                // ECANDIDUS 2020-11-11
                foreach (string sResource in assm.GetManifestResourceNames())
                {
                    if (sResource.IndexOf(key) != -1)
                    {
                        istr = assm.GetManifestResourceStream(sResource);
                    }
                }
            ...
            public static Stream GetResourceStream(string key)
        {
            ...
            try
            {
                ...
                // ECANDIDUS 2020-11-11
                istr = GetResourceStream(assm, key);
                //istr = assm.GetManifestResourceStream(key);
                // END ECANDIDUS
            }
            catch
            ...
            for(int k = 0; k < count; ++k) {
                ...
                try {
                    if(obj is Assembly) {
                        // ECANDIDUS 2020-11-11
                        istr = GetResourceStream((Assembly)obj, key);
                        ...
                        try {
                            // ECANDIDUS 2020-11-11
                            istr = GetResourceStream(Assembly.LoadFrom(dir), key);
